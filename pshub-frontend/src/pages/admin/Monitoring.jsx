      ...
      unitsData.forEach(item => {
        ...
        activeTransactionsForThisUnit.forEach((rent) => {
          ...
          if (!rent.waktu_mulai || !rent.waktu_selesai || statusDariDB === 'pending' || statusDariDB === 'booked' || statusDariDB === 'booking') {
            ...
            if (diffMs > 0) {
              // 🎯 REKAYASA SKALA VISUAL TESTING:
              // Karena di DB 1 jam = 1 menit (60000 ms), sisa milidetik yang ada dikalikan 60 
              // agar tampilan countdown jam tetap melaju cepat dan sinkron di layar admin
              const simulatedDiffMs = diffMs * 60;

              const diffHrs = Math.floor(simulatedDiffMs / 3600000);
              const diffMins = Math.floor((simulatedDiffMs % 3600000) / 60000);
              const diffSecs = Math.floor((simulatedDiffMs % 60000) / 1000);
        ...
        // ✅ FIX FINAL: Sisa slot kosong dihitung dari totalStok dikurangi slot aktif riil agar tidak terbuang ganda
