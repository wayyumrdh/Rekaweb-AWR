...
const Monitoring = () => {
  ...
  const fetchMonitoringData = async () => {
    try {
      ...
      const [unitRes, rentalRes] = await Promise.all([
        axios.get('http://localhost:5000/api/units', config),
        axios.get('http://localhost:5000/api/active-rentals', config)
      ]);
  ...
  const handleExtendBilling = async (e) => {
    ...
    try {
      await axios.put(`http://localhost:5000/api/admin/rentals/${selectedSlot.rentalId}/extend`, {
